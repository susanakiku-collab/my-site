...
aasync function signUp() {
  try {
    els.authMessage.textContent = "新規登録を実行中...";
    ...
    if (!window.APP_CONFIG) {
      els.authMessage.textContent = "config.js が読み込まれていません。";
      return;
    }

    if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
      els.authMessage.textContent = "Supabase設定が不足しています。";
      return;
    }
    ...
    if (error) {
      els.authMessage.textContent = `登録エラー: ${error.message}`;
      return;
    }
    ...
    if (data?.user) {
      ...
      console.log("profile upsert:", profileError);

      if (profileError) {
        els.authMessage.textContent = `登録は成功 / profiles保存エラー: ${profileError.message}`;
        return;
      }
    }

    els.authMessage.textContent = "新規登録に成功しました。";
  } catch (err) {
    console.error("signUp exception:", err);
    els.authMessage.textContent = `例外エラー: ${err.message}`;
  }
}
