...
function getManualCastCoords(){
  const lat = toNullableNum($("castLat")?.value);
  const lon = toNullableNum($("castLon")?.value);
...
async function fillCastGeoFromAddressOnly(){
  ...
  try{
    ...
    if($("castLat")) $("castLat").value = String(geo.lat);
    if($("castLon")) $("castLon").value = String(geo.lon);
...
async function fillCastGeoAndAreaFromAddress(){
  ...
  if(geo){
    ...
    if($("castLat")) $("castLat").value = String(geo.lat);
    if($("castLon")) $("castLon").value = String(geo.lon);
    if($("castArea")) $("castArea").value = area;
...
function bindCastAutoGeo(){
  const btn = $("btnCastAutoGeo");
  if(!btn){
    console.log("btnCastAutoGeo が見つかりません");
    return;
  }
  ...
  btn.onclick = async () => {
    ...
    try{
      const geo = await fillCastGeoFromAddressOnly();
      console.log("取得結果:", geo);
      ...
      console.error("btnCastAutoGeo error:", err);
...
function bindCastClearGeo(){
  const btn = $("btnCastClearGeo");
  if(!btn) return;

  btn.onclick = () => {
    if($("castLat")) $("castLat").value = "";
    if($("castLon")) $("castLon").value = "";
  };
}
...
function bindCastForm(){
  ...
  form.onsubmit = async e => {
    ...
    const existing = id ? getCastById(id) : null;

    let lat = existing?.lat ?? null;
    let lon = existing?.lon ?? null;
    let area = $("castArea").value.trim() || existing?.area || areaFromAddress(address);

    const manualGeo = getManualCastCoords();
